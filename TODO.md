...
## Revisión de código (2026-08-13)

- [x] **21. Barrido de limpieza sobre `app`, `components`, `lib` y `data`** — **hecho**. No apareció código muerto, exportaciones sin consumir ni duplicación: el objetivo original de la revisión salió limpio. Lo que sí salieron fueron **8 bugs de correctitud, todos corregidos**:
  1. `DishModal` — el reset de `imageLoaded` por efecto competía con el `onLoad` que `next/image` dispara desde un efecto de *layout* con imagen cacheada; el spinner podía quedarse girando para siempre. Agravado porque los 52 platos comparten el mismo placeholder, así que a partir del segundo todo es caché. Se sustituye el booleano por el `src` cargado, que no depende del orden de efectos.
  2. `lib/format.ts` — `toFixed(2)` daba `19.50€` (formato inglés) en una carta `lang="es"`. Ahora `Intl.NumberFormat("es-ES")` → `19,50 €`, con espacio no separable.
  3. `CategoryNav` — el atajo "Alérgenos" no armaba la supresión del scrollspy y el nav se sacudía al bajar. Se extrae `suppressSpyDuringScroll()` para que la use cualquier ancla, sea categoría o no.
  4. `CategoryNav` — el pill saltaba a la última categoría al tocar fondo. La causa real no era la rama `atBottom` sino el recálculo forzado al soltar la supresión, que pisaba la categoría recién pulsada; se elimina ese recálculo.
  5. `lib/menuData.ts` — un `null` o un string sueltos reventaban con un `TypeError` sin ubicación. Se añade `isPlainObject` en platos y categorías, con mensajes localizados. Importa porque cada carta se escribe a mano por cliente.
  6. `DishModal` — dos cierres seguidos (Escape mantenido, doble toque en el fondo) consumían dos entradas de historial y sacaban al usuario de la carta. Guarda `closingRef`.
  7. `DishModal` — el `pushState` perdía el marcador `__NA` de Next tras navegar por un ancla, y el botón "adelante" recargaba la página entera. Se propaga `...window.history.state`.
  8. `DishModal` — `aria-modal="true"` sin gestión de foco dejaba a un lector de pantalla en móvil sin alcanzar ni el botón de cerrar. Se mueve el foco al diálogo al abrir y se restaura al cerrar. **Distinto de la tarea 10**, que descartó los anillos de foco de teclado en escritorio, no el soporte para lectores de pantalla.
- [x] **22. Las anclas dejaban la tarjeta pegada a la cabecera** — **hecho**. `--header-h` pasa a llamarse `--anchor-offset` y vale el alto medido de la cabecera **+ 16px** (`ANCHOR_GAP`). El scrollspy usa ese mismo valor (+2px de epsilon subpíxel) en vez de una constante aparte: si no, la sección aterrizaría por debajo de la línea de disparo y al desplazarse a mano quedaría activa la categoría anterior.
